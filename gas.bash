...
if [ -z "$(command -v getopts 2> /dev/null)" ]; then
...
if [ -z "$(command -v home-manager 2> /dev/null)" ]; then
...
if [ -z "$(command -v nh 2> /dev/null)" ]; then
  echo "ERROR: nh not found"
  ((missingDependencies++))
fi
if [ -z "$(command -v alejandra 2> /dev/null)" ]; then
...
if [ "$upgradeFlakeLock" ] && [ -z "$updateOS" ] && [ -z "$updateHome" ]; then
  nix flake update
...
if [ "$updateHome" ]; then
  ...
  if [ "$upgradeFlakeLock" ]; then
    homeString="$osString --update"
  elif [ "$upgradeNixvim" ]; then
    homeString="$osString --update-input nixvim-config"
  fi
